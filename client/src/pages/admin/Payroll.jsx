...
export default function Payroll() {
  ...
  useEffect(() => {
    const fetchPayrolls = async () => {
      setLoading(true);
      try {
        const res = await axios.get("http://194.238.16.80:5000/payrolls");
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      try {
        const res = await axios.get("http://194.238.16.80:5000/admin/roles", { withCredentials: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      await axios.delete(`http://194.238.16.80:5000/payrolls/${id}`);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.put(`http://194.238.16.80:5000/payrolls/${editData._id}`, editData);
              ...
              try {
                ...
                const results = await Promise.all(
                  employeeList.map(emp =>
                    axios.post('http://194.238.16.80:5000/payrolls', {
                      employee: emp.name,
                      salary: emp.salary,
                      branch: emp.branch,
                      paymentDate,
