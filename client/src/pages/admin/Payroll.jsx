...
export default function Payroll() {
  ...
  useEffect(() => {
    const fetchPayrolls = async () => {
      setLoading(true);
      try {
        const res = await axios.get(`${API_BASE_URL}/payrolls`);
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/admin/roles`, { withCredentials: true });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      await axios.delete(`${API_BASE_URL}/payrolls/${id}`);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      await axios.put(`${API_BASE_URL}/payrolls/${editData._id}`, editData);
  ...
  return (
    <div className="w-auto space-y-5 py-6 bg-white">
              ...
              try {
                ...
                const results = await Promise.all(
                  employeeList.map(emp =>
                    axios.post(`${API_BASE_URL}/payrolls`, {
                      employee: emp.name,
                      salary: emp.salary,
                      branch: emp.branch,
                      paymentDate,
      ...
      <div className="flex justify-center mt-4">
        <button
          className={`px-6 py-2 rounded-full font-semibold text-white transition-all duration-150 ${selectedEmp ? "bg-[#57123f] hover:bg-[#7a1a59]" : "bg-gray-300 cursor-not-allowed"}`}
          disabled={!selectedEmp || autoPayLoading}
          onClick={async () => {
            ...
            try {
              ...
              const res = await axios.post(`${API_BASE_URL}/payrolls`, payload);
              setPayrolls((prev) => [res.data, ...prev]);
              toast.success("Salary paid successfully!");
              setSelectedEmp(null);
            } catch (err) {
              toast.error(err?.response?.data?.error || "Failed to pay salary");
            } finally {
              setAutoPayLoading(false);
            }
          }}
        >
          {autoPayLoading ? "Processing..." : "Auto Pay"}
        </button>
      </div>
    </div>
