...
const NewPayroll = () => {
    ...
    const fetchDropdowns = async () => {
      try {
        ...
        const [branchesRes, employeesRes, payersRes, monthsRes] = await Promise.all([
          axios.get('http://194.238.16.80:5000/branches'),
          axios.get('http://194.238.16.80:5000/employees'),
          axios.get('http://194.238.16.80:5000/payers'),
          axios.get('http://194.238.16.80:5000/payroll-months'),
        ]);
  ...
  useEffect(() => {
    ...
    if (id) {
      (async () => {
        setLoading(true);
        try {
          const res = await axios.get(`http://194.238.16.80:5000/payrolls/${id}`);
          setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      if (id) {
        await axios.put(`http://194.238.16.80:5000/payrolls/${id}`, formData);
        alert('Payroll updated successfully!');
      } else {
        await axios.post('http://194.238.16.80:5000/payrolls', formData);
